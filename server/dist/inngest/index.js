...
export const inngest = new Inngest({ id: "basket-delivery" });
// Low Stock Alert to Admin Email
const checkLowStock = inngest.createFunction({ id: "check-low-stock",
    name: "Low Stock Alert",
    triggers: [{ event: "inventory/stock.updated" }] }, async ({ event, step }) => {
...
// Monthly Offers Emails (1st of every month - payday)
const sendMonthlyOffers = inngest.createFunction({
    id: "send-monthly-offers",
    name: "Monthly Payday Offers",
    triggers: [cron("0 10 1 * *")]
}, async ({ step }) => {
    const { sales, users } = await step.run("fetch-sales-and-users", async () => {
        // Get top discounted products as featured sales
        ...
        return { sales: products, users: allUsers };
    });
    if (users.length === 0 || sales.length === 0) {
        return { skipped: true, reason: "No users or sales" };
    }
    let sentCount = 0;
