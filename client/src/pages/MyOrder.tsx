...
const MyOrder = () => {
  ...
  const tabs = ["All", "Placed", "Out of Delivery", "Delivery"]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {orders.map((order)=>(
              <Link key={order._id} to={`/order/${order._id}`} className="block max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {order.items.length > 4 && <div className="size-12 sm:size-16 rounded-lg bg-app-cream flex-center text-xs font-semibold text-app-text-light"> +{order.items.length - 4}
                  </div>
                  }
                </div>
                {/* total items & price */}
