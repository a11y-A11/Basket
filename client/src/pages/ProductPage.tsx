...
import { ArrowLeftIcon, HomeIcon, LeafIcon, MinusIcon, PlusIcon, ShoppingBagIcon, StarIcon } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-app-text-light mb-6">
          <Link to='/' className="hover:text-app-green transition-colors">
            <HomeIcon className="size-4" /></Link>
          <span>/</span>
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="relative flex-center p-8 md:p-12 min-h[320px] md:min-h[480px]">
              ...
              {product.isOrganic && (
                <span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-app-green text-white rounded-full">
                  <LeafIcon className="w-3 h-3" />Organic</span>
              )}
              { product.discount > 0 && (
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              ...
              {/* Rating */}
              {product.rating > 0 && (
                <div className="flex items-center gap-2 mv-5">
                  <div className="flex items-center gap-0.5">{[1,2,3,4,5].map((star)=> (<StarIcon key={star} className={`w-4 h-4 ${star <= Math.round(product.rating) ? "text-app-warning fill-app-warning" : "text-app-border"}`}/>))} </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={()=>{
                  ...
                  <ShoppingBagIcon className="w-4 h-4" />{inCart ? "Added to Cart" : "Add to Cart"}
                </button>
              </div>
            </div>
          </div>
