...
import OtpModel from "../../components/Delivery/OtpModel";
import CancelModel from "../../components/Delivery/CancelModel";
...
export default function DeliveryDashboard() {
    ...
    // OTP modal
    const [otpModel, setOtpModel] = useState<string | null>(null);
    ...
    const fetchOrders = async () => {
        setLoading(true)
        try{
            const {data} = await axios.get(`${API_URL}/delivery/my-delivery?status=${tab}`, getAuthHeaders())
        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {orders.map((order) => <DeliveryOrderCard key={order.id} order={order} tab={tab} handleUpdateStatus={handleUpdateStatus} setOtpModel={setOtpModel} setCancelModel={setCancelModel} />)}
                </div>
            )}
