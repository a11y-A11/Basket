...
const OrderTracking = () => {
  ...
  useEffect(()=>{
    api.get(`/order/${id}`).then((res)=> setOrder(res.data.order)).catch(()=>navigate("/order")).finally(()=> setLoading(false))
  },[id, navigate])
  ...
  useEffect(()=>{
    ...
    const fetchLocation = async () => {
      try {
        const { data } = await api.get(`/order/${id}/location`)
  ...
  return (
    <div className="min-h-screen mb-20 bg-app-cream">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <button onClick={()=> navigate("/order")} className="flex items-center gap-2 text-sm text-app-text-light hover:text-app-green mb-6 transition-colors">
          <ArrowLeftIcon className="size-4"/> Back to Orders</button>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl p-5">
              <h3 className="text-sm font-semibold text-app-green mb-3 flex items-center gap-2">Delivery Address</h3>
