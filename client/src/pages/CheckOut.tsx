...
const CheckOut = () => {
  ...
  const updateDeliveryAreaFromAddress = (selectedAddress: Address) => {
    const city = selectedAddress.city?.trim().toLowerCase();

    const newArea = city === "dhaka" ? "Dhaka" : "Outside";
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        shippingAddress: address, deliveryArea, paymentMethod
      }
  ...
  useEffect(()=>{
    if(user?.addresses?.length){
      ...
      updateDeliveryAreaFromAddress(defaultAddr);
    }
  }, [user])
    ...
    <div className="min-h-screen bg-app-cream">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <button onClick={()=> navigate(-1)} className="flex items-center gap-2 text-sm text-app-text-light hover:text-app-green mb-6 transition-colors">
          <ArrowLeftIcon className="size-4"/> Back </button>
            ...
            {steps.map((s, i) => (
              <div key={s.key} className="flex items-center gap-2">
                <button onClick={()=> setStep(s.key)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${step === s.key ? "bg-app-green text-white" : "bg-white text-app-text-light"}`}>
                  <s.icon className="size-4" /> {s.label} {i < steps.length - 1 && <ChevronRightIcon className="size-4 text-app-text-light" />}
                </button>
              </div>
            ))}
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2">
              {step === "address" && <CheckoutAddress address={address} setAddress={(selectedAddress: Address) => { setAddress(selectedAddress);  updateDeliveryAreaFromAddress(selectedAddress); }}  setStep={setStep} user={user}/>}
            ...
            <div className="bg-white rounded-2xl p-5 h-fit sticky top-24">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-app-text-light">Subtotal ({items.length} item)</span>
