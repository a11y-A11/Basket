...
interface DeliveryOrderCardProps {
    ...
    setOtpModel: (orderId: string) => void;
    setCancelModel: (orderId: string) => void;
}

export default function DeliveryOrderCard({ order, tab, handleUpdateStatus, setOtpModel, setCancelModel }: DeliveryOrderCardProps) {
        ...
        <div key={order._id} className="bg-white rounded-2xl border border-app-border overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-3">
                ...
                <div className="flex items-start gap-2 text-sm text-zinc-600">
                    ...
                    <p>{order.shippingAddress.address}, {order.shippingAddress.city}, {order.shippingAddress.district} {order.shippingAddress.zip}</p>
                </div>
            ...
            {tab === "active" && (
                <div className="px-5 py-3 border-t border-app-border flex flex-wrap gap-2">
                    ...
                    {order.status === "Out for Delivery" && (
                        <button onClick={() => setOtpModel(order._id)} className="px-4 py-2 text-sm font-medium bg-green-50 text-green-700 rounded-xl hover:bg-green-100 transition-colors flex items-center gap-1.5">
                            <CheckCircleIcon className="w-3.5 h-3.5" /> Mark Delivered
                        </button>
                    )}
                    {order.status !== "Delivered" && order.status !== "Cancelled" && (
                        <button onClick={() => setCancelModel(order._id)} className="px-4 py-2 text-sm font-medium bg-red-50 text-red-700 rounded-xl hover:bg-red-100 transition-colors flex items-center gap-1.5">
