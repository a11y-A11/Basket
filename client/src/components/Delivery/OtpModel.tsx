interface OtpModelProps {
    setOtpModel: (otpModel: string | null) => void;
    otp: string;
...
export default function otpModel({ setOtpModel, otp, setOtp, handleComplete, submitting }: OtpModelProps) {
    return (
        <>
            <div className="fixed inset-0 bg-black/40 z-50" onClick={() => setOtpModel(null)} />
            <div className="fixed inset-0 z-50 flex-center p-4">
                <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => { setOtpModel(null); setOtp(""); }} className="flex-1 py-2.5 text-sm font-medium text-zinc-600 bg-zinc-100 rounded-xl hover:bg-zinc-200 transition-colors">Cancel</button>
