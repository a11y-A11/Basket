...
interface AddressCardProps{
    addr: Address;
    onEditHandle: (addr: Address)=> void
            ...
            <div className='flex items-center gap-2 mb-1'>
                ...
                {addr.isDefault && (
                    <span className='flex-center gap-1 px-2.5 py-0.5 text-[10px] font-medium bg-app-green text-white rounded-full'>
                        <CheckIcon className="size-2.5"/> Default </span>
                )}
            </div>
            <p className="text-sm text-app-text-light">{addr.address},{addr.city}, <br />{addr.district}, {addr.zip}</p>
        </div>
    
        {/* Right - Action buttons*/}
        <div className="flex items-center gap-1">
            <button onClick={()=> onEditHandle(addr)} className="p-2 text-app-text-light hover:text-app-green hover:bg-app-cream rounded-lg transition-colors">
